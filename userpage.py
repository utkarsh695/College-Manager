...
class UserClass:
    ...
    def __init__(self,hwindow):
        ...
        self.window.title("My College Manager\ User")
        ...
        self.hdlbl = Label(self.window,text='User',background=mycolor2,font=('Cambria',20,'bold'))
    ...
    def updateData(self):
        if self.validationCheck()==False:
            return # end this function now
        try:
            qry = 'update usertable set password=%s, usertype=%s, pic=%s where username=%s'
            rowcount = self.curr.execute(qry ,( self.t2.get(), self.v1.get(),self.actualname, self.t1.get()) )
            self.conn.commit()
            if rowcount==1:
                #--------- update image ---------------
                if self.actualname!=self.oldname: # new image is given
                    ...
                    if self.oldname!=self.defaultname: # some image was given in past
                        import os
                        os.remove("user_images//"+self.oldname)
                #-----------------------------------------------------------
    ...
    def fetchData(self,pcolumn=None):
        ...
        try:
            ...
            if rowdata:
                ...
                self.actualname = rowdata[3]
                self.oldname = rowdata[3]
