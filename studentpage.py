...
class StudentClass:
    ...
    def __init__(self,hwindow):
        ...
        self.hdlbl = Label(self.window,text='Student',background=mycolor2,font=('Cambria',20,'bold'))
        ...
        # ------------------ table ---------------------
        self.mytable1 = Treeview(self.window, columns=['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8'], height=10)
        ...
        self.imgsize=150
        self.imglbl.place(x=x1+500,y=y1-self.imgsize,width=self.imgsize,height=self.imgsize)
        self.b6.place(x=x1+500,y=y1,width=self.imgsize,height=40)
    ...
    def updateData(self):
        ...
        try:
            ...
            if rowcount==1:
                #--------- update image ---------------
                if self.actualname!=self.oldname: # new image is given
                    self.img1.save("student_images//"+self.actualname)
    ...
    def fetchData(self,pcolumn=None):
        ...
        try:
            ...
            if rowdata:
                ...
                self.actualname = rowdata[8]
                self.oldname = rowdata[8]
                self.img1 = Image.open("student_images//"+self.actualname).resize((self.imgsize, self.imgsize))
                self.pimg1 = ImageTk.PhotoImage(self.img1)
                self.imglbl.config(image=self.pimg1)
            else:
